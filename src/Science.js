...
function Science() {
  ...
  const sciencemessagesRef = firestore.collection('sciencemessages');

  return (
    <div className="App-science">
      <section>
        {user ? <ChatRoom sciencemessagesRef={sciencemessagesRef} /> : <SignIn />}
      </section>
    </div>
  );
}
...
function ChatRoom({ sciencemessagesRef }) {
  const dummy = useRef();
  const query = sciencemessagesRef.orderBy('createdAt').limit(25);
  ...
  return (
    <>
      ...
      <form onSubmit={sendMessage}>
        <input value={formValue} onChange={(e) => setFormValue(e.target.value)} placeholder="ConnectIn: Science" />
